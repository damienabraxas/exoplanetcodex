...
- **RYA-1226 / RYA-1225** — near-UV Fe uncertainty close-out. **Six of the eight near-UV Fe rows now carry a complete, validated RYA-587 budget and publish** (`Fe.json` v1.221 -> v1.222); they are the first products in the feed to carry one at all. published A moves **0 of 160**. RYA-1225's adjudicated Fe II slope (-0.1100, post-RYA-1207 synthesis) is applied; near-UV Fe I was measured on its OWN pool on Sirius (kurucz2005 -0.1700, n=55 paired) with **no same-pool adoption**, and the two molecfit Fe I rows are HELD rather than salvaged because their pool moved (Fe I 3427.119 A rails at xi=1.10) — an incomplete budget must not publish.
- **RYA-1226 (Ryan's 2026-09-19 correction)** — 🔴 **blends is NOT an open uncertainty component and the 0.1721 dex ceiling is WITHDRAWN.** RYA-1190's own verdict is that this band's catalogued opacity is "already complete to the VALD threshold", so there are no un-modelled resolvable blends; what it found was an OPACITY deficit, and RYA-1204 corrected it and RYA-1207 wired it into the very synthesis these products run on. Both candidate numbers (the 0.1721 deficit, the ~0.028 payoff) are PRE-correction, so either bills the product for an error already removed. **The normalization term is now MEASURED**: RYA-846's sigma_delta at these 40 product lines, 0.070975 dex — the NET term, deconfounded from Wallace's own internal scatter, not the raw 0.1254 which would charge this product for the comparison reduction's noise. It supersedes the assumed 0.100 that RYA-846's artifact names as `assumed_term_dex`. The observed-holding-spread continuum floor (0.1075) is withdrawn as **circular** — half the kurucz2005-vs-molecfit spread prices each product from its sibling's answer — and it exceeded the real measurement. Bars **2.35-2.70x -> 1.83-2.00x**; bound share of variance **85% -> 81.6%**, measured share 6.3% -> **18.4%**.

### 2026-09-24 — RYA-1226 what remains owed

The grid-shift lever Ryan asked to have VERIFIED rather than assumed **exists**: RYA-1032 `atmosphere_nuisance`, atlas9 -> marcs-ges, family and NLTE held fixed, same 67-line pool, 0.004 dex. RYA-1032's STAGGER step is recorded as the different axis it is — 0.101 dex but `varied: dim`, the 1D -> <3D> dimensionality step this feed carries as its own treatment. ⚠️ **81.6% of the published near-UV variance is now a SINGLE term**: `profile_ew` at 0.2160, measured by RYA-1220 on **N I at 8216 A on solar_iag** — a different element, band and holding, whose own artifact declares `steps_are_adopted_uncertainties: false`. One near-UV half-width sweep on the Fe pool retires it and buys back four fifths of the bar. Also owed: sigma_delta for Kurucz2005-vs-1984KP specifically (RYA-1133 called it blocked on missing per-line files; RYA-908 emitted them the same day and 14 are in the tree, so it is RUNNABLE and would refine rather than unblock), and the molecfit Fe I xi bracket. 🔴 **RYA-587 still has no BOUNDED state** — a conservative ceiling and a measurement both read `DEFINED`, so a consumer cannot tell them apart from the schema alone.

## 2026-09-24 (later)

- **RYA-1223** — Fe publish: the live site refreshes from feed **v1.163 -> v1.222** and all five bands reconcile. 🔴 **No live abundance was ever wrong** — across the whole reconcile every stale row differed in `sigma_reported` / `sigma_xi` / `xi_state` ALONE; rows where `A` differs: **0**. The first pass corrected **20** stale bars and added **56** Reference-Grade products (the live site carried **no Reference tier at all** — RYA-1213 merged 09-16, the site was generated 09-11 — so it gained a tier rather than four bands); this pass adds RYA-1226's **6** migrated near-UV rows. ORPHAN 0 throughout. ⚠️ The ticket's smoke test expects the corrected bars to be LARGER: 16 widen and **4 NARROW**, and the 4 are the honest ones because RYA-1224 removed a *borrowed* sigma_xi from sub-floor holds. ⚠️ The live snapshot was **v1.163, not v1.158**.
- **RYA-1223 / RYA-1226** — near-UV is no longer held. Option B (2026-09-19) published the four complete bands first and held near-UV pending RYA-1226's component wiring; RYA-1226 landed, so the band reconciles normally and its page notice now states the real remaining caveat (two Fe I molecfit rows on a legacy bar pending their xi bracket; most of the quoted systematic on the migrated rows is a conservative profile-width term carried from another band) instead of "systematic under development". **No pre-opacity ~7.9xx value ships** — verified independently against both the feed and the live site; the earlier "stale site value" alarm was a FALSE POSITIVE that matched the WAVELENGTHS 5807.975/6077.976, not abundances.

### 2026-09-24 — RYA-1223 what it deliberately did not do

Everything is generated by the site's own `scripts/generate_fe_publication.py`; no page hand-edited and no value typed (RYA-914). The per-band best-value headline selection was left exactly as already instructed — it renders the VIS Reference Amarsi 3D-NLTE anchor and **7.466 is not the headline**. 🔴 **Budget completeness does NOT pass feed-wide and does not pass differentially**: `sigma_syst_components` is `(published_syst, sigma_xi)` on 154 of 160 products with no named stellar-parameter term in any band, the sourced budget reaches no product path (RYA-1112 F1 standing), and `audit_uncertainty_rya587.py` reported "full-contract complete set: []" with 160/160 failing before RYA-1226 — six now pass. Ryan ruled PROCEED on the grounds that the reconcile is strictly more honest than a live site carrying 16 too-small bars and 7 xi-less products. Teff/logg stays owed to **RYA-1120**, canonical budgets to **RYA-587**. Also recorded, not fixed: the near-UV `opacity_note` carried in `Fe.json` still asserts "the headline, which rests on the VIS anchor 7.466" — stale prose on 5 products that no script writes, so the fix belongs to the RYA-1178 emitter path.
