...
| 6 | Model registry | [`data/catalog/model_registry.csv`](data/catalog/model_registry.csv) (+ collision note [`docs/catalog/model_registry_notes.md`](docs/catalog/model_registry_notes.md)) | the canonical roster of the 8 abundance-software models — token, physics axes, nickname, pair rule. **Live product COUNTS are not here**; they belong to RYA-1015 | a model is added, retired, or its stored token changes | **NATIVE** — `stored_token` is READ from `band_products.TREATMENTS`; `scripts/verify_model_registry.py` loud-fails if a token is not in the code |
