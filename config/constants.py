...
def _resolve_spectra_ext_dir() -> Path:
    """RYA-1140 — the `exoplanetcodex-data` spectra tree, resolved by CAPABILITY.

    Eleven sites spelled `SPECTRA_EXT_DIR`
    inline. That is the worktree's parent, so it is canonical only while every
    worktree shares one -- RYA-1090's mechanism, eleven times over.

    🔴 AND IT CURRENTLY RESOLVES TO A DIRECTORY THAT DOES NOT EXIST. From a worktree
    under ~/codex it yields ~/codex/data/spectra/exoplanetcodex-data, which is absent;
    the tree is at ~/Projects/data/... on this machine. The readers guard with
    `skipif(not _HAS_DATA)`, so the failure never surfaced: a MISSING DEPENDENCY
    became what reads as MISSING DATA, and the tests have been silently SKIPPING. A
    skipped test is in neither the pass set nor the fail set (RYA-1035).

    ⚠️ NOT registered in path_register.yaml, and deliberately not folded into
    `data.spectra`: every registered spectra entry is rooted at `data`
    (`SIRIUS_DATA_ROOT`) with no `exoplanetcodex-data` component, so they are
    DIFFERENT TREES. Re-pointing these eleven readers at `data.spectra` would change
    which files they open on an unverified assumption -- a value change wearing a
    path-fix costume, which RYA-1140's firewall forbids.

    The predicate names an actual subtree rather than `is_dir()`: a directory that
    merely exists is the same silent absence one level down.
    """
    from pipeline.canonical_checkout import (resolve_external,
                                             CANONICAL_WORKTREE_PARENT)
    tail = ('data', 'spectra', 'exoplanetcodex-data')

    def _serves(p: Path) -> bool:
        return p.is_dir() and any((p / n).is_dir()
                                  for n in ('Procyon', 'Solar Calibration'))

    candidates = [ROOT.parent.joinpath(*tail),                 # historical, first
                  CANONICAL_WORKTREE_PARENT.joinpath(*tail),
                  Path.home().joinpath('Projects', *tail)]
    try:
        found = resolve_external('exoplanetcodex-data spectra tree',
                                 env='CODEX_SPECTRA_EXT_ROOT', candidates=candidates,
                                 serves=_serves, purpose='raw atlas / UVES spectra')
    except Exception:
        found = None
    # Unresolved keeps the historical value so `skipif(not _HAS_DATA)` still behaves
    # exactly as before on a machine that genuinely lacks the tree. Resolution
    # improves; nothing starts failing for want of data that was never there.
    return found or ROOT.parent.joinpath(*tail)


SPECTRA_EXT_DIR = _resolve_spectra_ext_dir()
...
PATHS = {
    ...
    'solar_spectra'     : SPECTRA_EXT_DIR / 'Solar Calibration' / 'archive',

    # Procyon HARPS spectra (external data — relative to repo parent)
    'procyon_spectra'   : SPECTRA_EXT_DIR / 'Procyon' / 'Procyon Harps',
...
def _resolve_ispec_dir() -> Path:
    """RYA-1140 — iSpec by CAPABILITY, not by the worktree's parent.

    🔴 THIS ONE LINE INVERTED A MERGE VERDICT. It was
    `Path(_os.environ.get('ISPEC_DIR', str(ROOT.parent / 'ispec')))`, and
    `abundances_derive` puts the result straight onto `sys.path`. From a worktree not
    sited beside the iSpec checkout, `import ispec` fails, 25 test modules fail to
    IMPORT, and the run reports COLLECTION ERRORS rather than failures -- which reads
    as a clean baseline. RYA-1138 caught eleven real regressions about to be reported
    as an improvement on exactly that.

    It is also RYA-1090's mechanism verbatim: canonical only while every worktree
    shares one parent, and silently wrong the moment they do not -- which is the move
    that created RYA-1090.

    ⚠️ THE FALLBACK IS DELIBERATE AND MUST STAY. Nothing here raises when iSpec is
    absent: this module is imported by effectively everything, and a machine without
    the engine (CI containers, a docs build) must still be able to `import
    config.constants`. Resolution improves; import-time behaviour does not change. The
    loud failure belongs at the point of USE -- `require_ispec_dir()` below -- not at
    the point of import.
    """
    from pipeline.canonical_checkout import (resolve_external,
                                             CANONICAL_WORKTREE_PARENT)
    ...
    def _serves(p: Path) -> bool:
        # The package itself, not merely a directory named 'ispec'. An empty or
        # half-cloned directory that satisfies `.is_dir()` is the same silent-absence
        # failure one level down.
        return (p / 'ispec' / '__init__.py').is_file()

    candidates = [ROOT.parent / 'ispec',            # the historical derivation, first
                  CANONICAL_WORKTREE_PARENT / 'ispec',
                  CANONICAL_WORKTREE_PARENT / 'engines' / 'ispec_src']
    try:
        found = resolve_external('iSpec', env='ISPEC_DIR', candidates=candidates,
                                 serves=_serves, purpose='synthesis engine + input/')
    except Exception:
        found = None
    # Unresolvable is NOT an error here -- see the docstring. Keep the historical
    # path so the value is stable and every existing message still points somewhere
    # recognisable.
    return found or Path(_os.environ.get('ISPEC_DIR', str(ROOT.parent / 'ispec')))


ISPEC_DIR = _resolve_ispec_dir()




def require_ispec_dir(context: str = "") -> Path:
    """`ISPEC_DIR`, or a LOUD failure naming every root that was tried.

    Use this wherever iSpec is actually needed. The point of RYA-1140 is that a
    missing engine must not present as missing data.
    """
    if (ISPEC_DIR / 'ispec' / '__init__.py').is_file():
        return ISPEC_DIR
    from pipeline.canonical_checkout import (DependencyNotServedError,
                                             CANONICAL_WORKTREE_PARENT)
    raise DependencyNotServedError(
        f"iSpec is not installed at {ISPEC_DIR}{' (' + context + ')' if context else ''}.\n"
        f"  Tried: {ROOT.parent / 'ispec'}, {CANONICAL_WORKTREE_PARENT / 'ispec'}, "
        f"{CANONICAL_WORKTREE_PARENT / 'engines' / 'ispec_src'}\n"
        f"  Set $ISPEC_DIR. Failing here rather than letting `import ispec` become a "
        f"collection error that reads as a clean test baseline (RYA-1138/1140).")
...
def _expand_repo_parent(raw: str, repo: Path) -> str:
    """RYA-1140 — expand `{repo_parent}` to the parent that ACTUALLY HAS the thing.

    🔴 THE REGISTER'S OWN TOKEN CARRIED THE DISEASE IT EXISTS TO CURE. `codex_root()`
    is the mechanism that centralises path resolution so nothing has to hardcode a
    location -- and it expanded `{repo_parent}` to `repo.parent`, the parent of
    whichever worktree happened to import it. So a path could route correctly through
    the register and still land on RYA-1090's mechanism, one level further in and much
    harder to see, because the calling site looks like it is doing the right thing.

    ⚠️ THE TOKEN WAS A GOOD IDEA AND IS KEPT. It exists (RYA-810) to avoid baking a
    username into git -- it replaced 15 absolute /Users/<name>/... literals that were
    unrunnable by anyone else and a disclosure in a shared repo. Those properties are
    preserved: no username, no configuration required. What changes is that the
    derived parent is now VERIFIED rather than assumed, and the search widens to the
    other parents worktrees have actually lived under.

    Unresolved falls back to `repo.parent`, exactly the old behaviour, so a path that
    refers to a directory not yet created still resolves and nothing starts raising at
    import time.
    """
    if '{repo_parent}' not in raw:
        return raw
    from pipeline.canonical_checkout import CANONICAL_WORKTREE_PARENT
    seen, candidates = set(), []
    for cand in (repo.parent, CANONICAL_WORKTREE_PARENT, Path.home() / 'Projects'):
        if cand not in seen:
            seen.add(cand)
            candidates.append(cand)
    for cand in candidates:
        expanded = raw.replace('{repo_parent}', str(cand))
        probe = Path(_os.path.expanduser(_os.path.expandvars(
            expanded.replace('{repo}', str(repo)))))
        if probe.exists():
            return expanded
    return raw.replace('{repo_parent}', str(repo.parent))
...
def codex_root(name: str) -> Path:
    ...
    r = reg['roots'][name]
    raw = str(_os.environ.get(r['env'], r['default']))
    repo = Path(__file__).resolve().parents[1]
    raw = _expand_repo_parent(raw, repo).replace('{repo}', str(repo))
