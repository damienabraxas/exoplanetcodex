...
def air_to_vac_A(wl_air_A):
    """Birch & Downs (1994) air->vacuum (Angstrom), VALD3/iSpec convention."""
    s2 = (1.0e4 / np.asarray(wl_air_A, dtype=float)) ** 2
    n = 1.0 + 0.0000834254 + 0.02406147 / (130.0 - s2) + 0.00015998 / (38.9 - s2)
    return np.asarray(wl_air_A, dtype=float) * n


def vac_to_air_A(wl_vac_A):
    """Birch & Downs (1994) vacuum->air (Angstrom)."""
    s2 = (1.0e4 / np.asarray(wl_vac_A, dtype=float)) ** 2
    n = 1.0 + 0.0000834254 + 0.02406147 / (130.0 - s2) + 0.00015998 / (38.9 - s2)
    return np.asarray(wl_vac_A, dtype=float) / n
...
def extract_kp_diagnostics():
    ...
    for name, el, line_A, lo_A, hi_A, consumer in DIAGNOSTICS:
        ...
        df['wavelength_vac_A'] = np.round(air_to_vac_A(df['wavelength_air_A'].values), 4)
...
def extract_uv_composite():
    ...
    df = pd.DataFrame({
        ...
        'wavelength_air_A': np.round(vac_to_air_A(w_vac), 4),
