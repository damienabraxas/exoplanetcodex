...
def main():
 ...
 for c in can:
  try:idx[(c['species'].replace(' ',''),round(float(c['wavelength_air_A']),1))].append(c)
  except:pass
 ...
 for band,name in BANDS.items():
  for raw in (SRC/name).read_text(errors='replace').splitlines():
   ...
   w=fl(raw,0,12);ep=fl(raw,18,27);flags={k:raw[v] for k,v in GD.items()}
   hits=[]
   for b in (round(w-.1,1),round(w,1),round(w+.1,1)):
    for c in idx[(sp,b)]:
     try:
      if abs(float(c['wavelength_air_A'])-w)<=.05 and abs(float(c['excitation_potential_eV'])-ep)<=.01:hits.append(c)
     except:pass
   c=hits[0] if len(hits)==1 else {}; grade=c.get('gf_tier',''); gradeable=grade=='LAB' or bool(c.get('nist_grade',''))
   ...
   assessed=flags['robust'] in 'YN'; robust=flags['robust']=='Y'; unassessed=not assessed
