...
#: all eight. Ryan's ruling (2026-08-29) splits them, and the split is by ROUTE+ATMOSPHERE,
#: never by substring:
#:
...
#:       input axis of the Amarsi MLP (training box 0-3 km/s; pipeline/amarsi3d.py hands it
#:       to `classify_line` and `_apply_aberr_to_line`), and measured on its own optical
#:       Fe I set at xi = 1.0 +/- 0.2912 the CORRECTION moves by d(aberr)/dxi = +0.0985
#:       dex/(km/s) (112 of 117 lines, none exactly zero, in-domain count identical at all
#:       three xi so it is not a selection effect). The exemption rests on the published
#:       quantity being a SUM, A(3D-NLTE) = A(1D-LTE)(xi) + aberr(xi), whose halves move
#:       oppositely -- so the correction tracking xi is the baseline's xi-dependence being
#:       undone, not a second one being added.
#:
...
#:       The mean atmosphere averages the velocity structure OUT, so the route still runs
#:       on an inherited xi. That ticket forbids this exemption in writing.
...
def audit() -> dict:
    ...
    for p in vis:
        ...
        products.append({
            ...
            # RYA-1120: a PRIOR EXPECTATION from the line set, never a verdict. The
            # tier split is a real property (feature depth), so it says where xi is
            # LIKELY to bite -- it does not say whether the term applies, because that
            # is a measurement nobody has made on this pool yet.
            ...
            "xi_applicability": (XI_NOT_APPLICABLE if is_full_3d(p["treatment"])
                                 else XI_APPLICABILITY_UNRESOLVED),
        })
    products.sort(key=lambda r: -r["honest_total_dex"])

    # RYA-1120: FULL 3D only -- the <3D> mean is counted as APPLYING (RYA-1099).
    n_na = sum(1 for r in products if is_full_3d(r["treatment"]))
    return {
        "ticket": "RYA-1112",
            ...
            "F1_stellar_parameter_terms_absent_from_published_sigma_syst": {
                ...
                "products_over_dig_in_if_FeI_vmic_folded_in": sum(
                    ...
                    + sum(1 for r in products
                          if (r["ion"] != "I" or is_full_3d(r["treatment"]))
                          and r["over_dig_in"]),
            },
        ...
        "n_products_where_xi_is_not_applicable_full_3d_only": n_na,
        "xi_applicability_basis": (
            "RYA-1120: applicability is a MEASUREMENT, never an engine name. The prior "
            "`\"3D\" in treatment` rule was refuted on BOTH halves -- RYA-1099 measured "
            "the <3D> mean at xi=0 as +0.137 dex WORSE, and vmic is an explicit input "
            "axis of the Amarsi full-3D MLP (measured d(aberr)/dxi = +0.0985 dex/(km/s) "
            "on its own line set, 112 of 117 lines, none zero)."),
        "products": products,
    }
