...
def test_fkm_profiles_fail_loud_no_silent_default():
    # RYA-277 F/K/M are TODO stubs -> must fail loud, never default silently.
    assert C.ACCEPTANCE_PROFILES['F'] is None
    assert C.ACCEPTANCE_PROFILES['K'] is None
    assert C.ACCEPTANCE_PROFILES['M'] is None
    for t in ('F', 'K', 'M'):
...
def test_star_spectral_type_mapping():
    ...
    assert C.STAR_SPECTRAL_TYPE['procyon'] == 'F'         # F is a stub -> procyon keeps legacy


def test_no_hardcoded_scatter_threshold_in_gate_script():
    # The gate must read the threshold from the profile, not hardcode it.
    src = (ROOT / 'scripts' / 'validate_fe_rya238.py').read_text()
    assert "fe1_scatter_threshold('G')" in src
    assert '0.1398' not in src                            # no hardcoded copy in the script
