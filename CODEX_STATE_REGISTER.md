...
| Engine B substrate (NLTE synthesis engines) | Corrects the RYA-529 PySME-wholesale row. Production NLTE is universally a 1D delta CSV added to the TS/EW LTE abundance (**Engine A**; `nlte_corrections` + `nlte_cno`) — there is **no bsyn in-synthesis NLTE** anywhere in production. RYA-402's "two-family" split maps how the Engine-A delta is DERIVED: **Family A** (O/Na/Mg/Si/Ca/Ti/Mn/Co/Ni/Sr/Ba) was planned as TS-native Gerber-2022 (A&A 666 A18) but **never executed** — Na/Mg/Si/N from Amarsi-2020 via **PySME** (RYA-410/526), Ca/Ti/Mn via **MPIA**, Ba via **Korotin**, Sr via **INSPECT**, C/O via **nlte_cno** (Amarsi-2019), Co/Ni **LTE**; **Family B** (Al/K/S) via **PySME** on Amarsi `.grd` (Zenodo 3982506; Cu PySME-validated but unregistered). **Engine B = Turbospectrum synthesis = LTE-only for all 27**; the Gerber-2022 TS-native departure deck (NLTEINFOFILE/MODELATOMFILE/DEPARTUREFILE) is **NOT built, NOT provisioned** (empty `gerber_ts/` on Sirius) | Engine-A 1D-NLTE: VALIDATED (RYA-529 Na PySME leg δ −0.129 vs −0.107 on Sirius; Ca/Ti/Cr/Mn/Ba/Sr published grids; C/O nlte_cno). **Engine-B TS-native NLTE synthesis: OWED for all 27** — the genuine RYA-525 gap (from-scratch RT-engine build, not a re-provision). Na TS-Gerber gate (the leg RYA-529 skipped) **COULD NOT RUN** — the path does not exist. Per-element map → `data/audit/rya531_engineB_map/engineB_per_element.md` | RYA-402 / RYA-529 / RYA-531 | SETTLED-WITH-CAVEAT — Engine-A 1D-NLTE multi-source + validated; **Engine-B TS-native NLTE OWED/loud** (unbuilt; Na TS-Gerber gate could not run — TS-native NOT claimed SETTLED). RYA-531 anti-reinvention finding: TS-native was a deliberately-abandoned derivation route (RYA-402 skipped Option 1), not a re-provision | grid/atom set changes; engine regression; either Na gate (PySME or TS-Gerber) reopens; Engine-B NLTE built |
