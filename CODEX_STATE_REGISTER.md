...
| Sr II | WORKING = Bergemann 2012a/INSPECT → `Sr_Bergemann2012_INSPECT.csv` (applied). PRIMARY = Mashonkina 2022/INASAN NOT vendored (WAF-403, manual pull owed) | 4077 primary / 4215 Fe I-blended cross-check; [Fe/H] ceiling +0.0 → solar/55 Cnc out-of-hull, loud | RYA-421/428/433 | PARTIAL — grid registered; primary pull + Sr II measurement owed (GET-DATA) — **surfaced by RYA-530** |
...
_NLTE-grid notes (RYA-530): **Cu** — departure inputs staged (`amarsi_galah/Cu_caliskan2024.*`) but the grid is **NOT committed to `nlte_grids/` and NOT wired**; blocker is measured-line quality (5 lines red_chi2 8–127), not the grid (RYA-466). **V** — genuine void, no public NLTE grid → HFS-LTE carry-forward (RYA-470/526)._
...
| 1D→3D correction (applied delta) | `A(3D-NLTE)=A(1D-NLTE)+δ3d` from vendored `data/threed_grids/solar3d_metals_rya399.csv` (Si −0.01 Amarsi&Asplund 2017; Ti +0.06, Cr +0.03 Scott 2015); `3D_unavailable` flag if no node; wired via `nlte_corrections.apply_threed_corrections` | Si/Ti/Cr (+ C/O via nlte_cno) | RYA-399 (`a3c5796`) | SETTLED (applied) — **surfaced by RYA-530**; FINDING: 3D is NOT the lever for the Si/Ti/Cr residual (\|δ\|≤0.1); **distinct** from the still-absent 3D-RT synthesis engine (RYA-444). Dead `CORRECTIONS_3D` scalar dict superseded but not yet removed (RYA-474 owed) |
