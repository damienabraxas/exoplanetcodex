...
| Engine B substrate (NLTE synthesis engines) | Corrects the RYA-529 PySME-wholesale row. Production NLTE is universally a 1D delta CSV added to the TS/EW LTE abundance (**Engine A**; `nlte_corrections` + `nlte_cno`) — there is **no bsyn in-synthesis NLTE** anywhere in production. RYA-402's "two-family" split maps how the Engine-A delta is DERIVED: **Family A** (O/Na/Mg/Si/Ca/Ti/Mn/Co/Ni/Sr/Ba) was planned as TS-native Gerber-2022 (A&A 666 A18) but **never executed** — Na/Mg/Si/N from Amarsi-2020 via **PySME** (RYA-410/526), Ca/Ti/Mn via **MPIA**, Ba via **Korotin**, Sr via **INSPECT**, C/O via **nlte_cno** (Amarsi-2019), Co/Ni **LTE**; **Family B** (Al/K/S) via **PySME** on Amarsi `.grd` (Zenodo 3982506; Cu PySME-validated but unregistered). **Engine B = Turbospectrum synthesis = LTE-only for all 27** in production; the Gerber TS-native departure deck (NLTEINFOFILE/MODELATOMFILE/DEPARTUREFILE) is **being built (RYA-533)** — Turbospectrum_NLTE (BSYN v20.1) compiled on Sirius + Na Gerber-2023 (A&A 669 A43) grid/atom provisioned; deck driver + Na gate PENDING | Engine-A 1D-NLTE: VALIDATED (RYA-529 Na PySME leg δ −0.129 vs −0.107 on Sirius; Ca/Ti/Cr/Mn/Ba/Sr published grids; C/O nlte_cno). **Engine-B TS-native NLTE synthesis: BUILD IN PROGRESS (RYA-533)** — the genuine RYA-525 gap (from-scratch RT-engine build). Step-0 forensics = NOT-BUILT on all 5 channels (RYA-361 scoped it, never executed). This session: TS_NLTE v20.1 compiled on Sirius + Na Gerber-2023 grid(15.9 GB)/atom/aux provisioned (md5-pinned, Sirius-only); deck driver + Na gate PENDING → still OWED until the gate passes (no SETTLED claim). Per-element map → `data/audit/rya531_engineB_map/engineB_per_element.md`; build status → `data/audit/rya533_ts_gerber_build/build_status.md` | RYA-402 / RYA-529 / RYA-531 / RYA-533 | SETTLED-WITH-CAVEAT — Engine-A 1D-NLTE multi-source + validated; **Engine-B TS-native NLTE OWED — build in progress (RYA-533)**: engine compiled + Na grid provisioned on Sirius, deck driver + Na gate PENDING (NOT claimed SETTLED until the gate passes). RYA-531 finding: TS-native was a deliberately-abandoned derivation route (RYA-402 skipped Option 1); RYA-533 is now building it | grid/atom set changes; engine regression; either Na gate (PySME or TS-Gerber) reopens; Engine-B NLTE gate passes |
...
## Changelog
- **v9** (2026-07-06) — RYA-533: TS-native Gerber NLTE synthesis deck (Engine-B NLTE for RYA-525). **Forensics-first verdict = NOT-BUILT** on all 5 channels (0a test-tell: no TS-Gerber reproduction test, only a docstring; 0b git: no surviving 402 branch, merged via the PySME line; 0c tree: `synth_ew_nlte_vs_lte` = NotImplementedError, no bsyn deck; 0d store: empty `gerber_ts/`, no TS_NLTE build; 0e reg: no Gerber flag — RYA-361 scoped it, Backlog/never-started). TS binary hooks ≠ our deck. **Build STARTED on Sirius (multi-session):** Turbospectrum_NLTE **BSYN v20.1 compiled** (gfortran 15.2), deck format recovered from the repo docs, **Na Gerber-2023 grid (15.9 GB) + model atom + aux provisioned** (md5-pinned, Sirius-only `/mnt/codex-data/grids/nlte/gerber_ts/`). Deck driver + 4D departure interp + **Na gate PENDING** → Engine-B NLTE stays **OWED, NOT SETTLED** (no gate = no claim). Resolved the 2022-vs-2023 citation: arXiv 2206.00967 = **A&A 669, A43**. Build status + resume point → `data/audit/rya533_ts_gerber_build/build_status.md`.
