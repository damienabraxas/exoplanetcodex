...
**Version: v7** · _Last updated: 2026-07-06 · By: Mr. Code + Ryan (RYA-529: RYA-402 b-factor NLTE synthesis re-provisioned + re-validated on Sirius — Engine B substrate for the two-engine floor; the correction engine is PySME (native `.grd` consumer), TS-`bsyn` deck stays NotImplementedError; INSPECT Na −0.107 reproduced (median −0.129) on Sirius via `venv_pysme`; Amarsi-2020 base-13 grids (Zenodo 3982506) Sirius-only; unblocks RYA-525)._
...
| TS NLTE synthesis (Engine B substrate) | RYA-402 b-factor NLTE capability re-provisioned + re-validated on **Sirius**. Correction engine = **PySME** (native consumer of the Amarsi `.grd` departure grids); RYA-402's TS-`bsyn` deck stays a documented `NotImplementedError`, so Turbospectrum remains the LTE spectral-synthesis engine and PySME derives the NLTE Δ = A(NLTE) − A(LTE) applied to the TS/EW LTE baseline (Ryan's 2026-06-21 RYA-402 decision). Reproduces the INSPECT Na anchor through the synthesis path: median Δ **−0.129** (5682.633 −0.121 / 5688.205 −0.137) vs **−0.107 ± 0.03 → PASS** on Sirius, matching the Mac RYA-402 result to ≤0.001/line | NLTE-synth READY for the vendored b-factor-grid elements = Amarsi-2020 **base-13** (H, Li, C, N, O, Na, Mg, Al, Si, K, Ca, Mn, Ba; Zenodo **3982506**, on `/mnt/codex-data/grids/nlte/amarsi_galah/`, Sirius-only per RYA-526); Cu (Caliskan)/S (Amarsi-2025) grids from RYA-402 NOT re-provisioned on Sirius — acquire if needed. LTE-synth elsewhere (V void, Zr dominant-ion, gf-limited set) | RYA-529 (re-provision of RYA-402 on Sirius) | SETTLED — Na gate green on Sirius (`venv_pysme` py3.12.13 / numpy 2.2.6 / PySME 1.0.2) | grid set changes / PySME-NLTE regression / Na gate reopens |
