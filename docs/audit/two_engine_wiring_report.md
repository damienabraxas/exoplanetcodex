...
`both` **10** · `A_only` **0** · `B_only` **9** · `neither` **8** — 27 species

## The headline

**8 of 27 species have no Engine B.** Of those, **4 are synthesis-required** — elements whose raw-EW leg is deliberately suppressed by RYA-520, so Engine B is not their second opinion, it is their *only* leg. Those rows are reporting on one engine or on nothing.

| species | treatment | why Engine B is missing |
|---|---|---|
| N I | `per_region_source` | `NO_MODEL_ATOM` |
| Sc II | `HFS_sum` | `LTE_ONLY_BY_DESIGN` |
...
Species that produce **no two-engine record at all**, and therefore never reach the guard: **P I, N I, Al I, K I, Y II, Sc II, Eu II, Zr II**.
...
### Measured, but invisible to the floor (4)
...
| Sc II | `neither` | ✗ | ✗ | DELIBERATELY_SKIPPED | LTE_ONLY_BY_DESIGN | `HFS_sum` | **YES** |
...
- **`NO_MODEL_ATOM`** (N I, Al I, K I, Y II) — no validated Engine-B NLTE atom for the species. Needs atom sourcing + an RYA-534/548-style anchor validation BEFORE wiring is meaningful.
...
| Sc II | **BLOCKS BETA** — synthesis-required with no Engine B (`LTE_ONLY_BY_DESIGN`). Needs the underlying gap closed first. |
...
A: raw-EW leg suppressed by RYA-520 (required_treatment=HFS_sum) — Engine B is this species' only valid leg. B: ratified LTE-only by RYA-460 — no Engine-B atom is supposed to exist for this species; this is a settled disposition, NOT refinement debt; ⚠ MEASURED OFF-ORCHESTRATOR: phase_c carries A=3.203 via "kittpeak: Sc II 4246 (blue-edge, HFS)" — a channel the two-engine floor never sees, so this value has NO cross-engine confirmation and never can until it is wired
