...
_Generated 2026-07-27 by scripts/phase_c_verdict_rya371.py. Validate-don't-tune: classification only, no correction fitted to the anchor._

_RYA-460: Kitt Peak Solar Flux Atlas wired in (N + P/K/Co/Sc). Leg validated by overlap: PASS._

## Verdict counts

- **PASS**: 6  (prior 3, diff +3)
...
| Al | 6.43 |  |  |  | 1 | wired | **CURATION-OWED** | harps-measured (EW pool) | EW: 1 curated line(s); value HELD at gold tier 'owed' (RYA-522) — not a graded-cull blank |
| Ca | 6.30 |  |  |  | 2 | wired | **CURATION-OWED** | harps-measured (EW pool) | EW: 2 curated line(s); value HELD at gold tier 'owed' (RYA-522) — not a graded-cull blank |
| Na | 6.24 |  |  |  | 2 | wired | **CURATION-OWED** | harps-measured (EW pool) | EW: 2 curated line(s); value HELD at gold tier 'owed' (RYA-522) — not a graded-cull blank |
| Ni | 6.20 |  |  |  | 2 | - | **CURATION-OWED** | harps-measured (EW pool) | EW: 2 curated line(s); value HELD at gold tier 'owed' (RYA-522) — not a graded-cull blank |
...
| Ti | 4.97 |  |  |  | 10 | wired+3D | **CURATION-OWED** | harps-measured (EW pool) | EW: 10 curated line(s); value HELD at gold tier 'owed' (RYA-522) — not a graded-cull blank |
| Co | 4.94 | 4.965 | +0.025 | 0.07 | 5 | - | **PASS** | synthesis: Co I red HFS (RYA-564) | synthesis: Co I red HFS-resolved flux fit (5 lines, HARPS; blends modelled) + per-line Gerber 1D-NLTE (RYA-534 deck) |
...
| Sr | 2.83 |  |  |  | 1 | wired | **CURATION-OWED** | harps-measured (EW pool) | EW: 1 curated line(s); value HELD at gold tier 'owed' (RYA-522) — not a graded-cull blank |
| Zr | 2.59 |  |  |  |  | - | **CURATION-OWED** | harps-measured (EW pool) | EW present; no independent-gf line survives the graded cull |
| Ba | 2.27 | 2.410 | +0.140 | 0.07 | 1 | wired | **CURATION-OWED** | synthesis: Ba II 5853 HFS (RYA-559) | synthesis: Ba II 5853.668 HFS-resolved LTE COG (Turbospectrum, VALD3 gf loggf -1.000) + Engine-A Korotin2015 1D-NLTE delta |
...
- **Co** — MEASURED on clean RED Co I lines via HFS-resolved synthesis — the EW path cannot reach Co (hyperfine-split, RYA-354/466) and the previous number came from the blue-edge Co I 3845, now DEMOTED to diagnostic-only (that +1.188 was an SNR~24/chi2r~3100 artifact, not solar Co). Per-line A(Co) [4813.476=4.974, 5212.688=4.847, 5352.040=4.875, 5483.354=4.965, 6771.034=5.008] -> A(Co) 4.965 (+0.025 vs Asplund 4.94), scatter 0.069; per-line 1D-NLTE from the RYA-534-validated Gerber TS-native grid (median +0.0951 vs the Bergemann+2010 anchor +0.100 ± 0.12 — CONSISTENT; read, never fitted). Cross-checks — gf-'agreed' lines only: 4.925 (n=4, scatter 0.077); IAG FTS arm: 4.939 (n=5, scatter 0.056). RECONCILES within tol on a validated leg: the red-line value replaces the artifact and Co clears. Scale caveat (RYA-561/593): this is a 1D-NLTE value against a 3D-NLTE Asplund reference — the un-applied 3D term is folded into the offset; the class-wide 3D-metals correction is post-Beta (RYA-593).
...
- **Mg** — solar EW measured + matched in linelist_solar, but the RYA-398 graded-gf firewall (wired into the default run, RYA-456) culls every line — zero independent-gf survivors (pool gf is Kurucz/ungraded, or every line fails SAT/HIERR/BLEND). gf-data-limited → RYA-161/162 (differential survey). NLTE grid available (Mg_Amarsi2020_PySME.csv).
...
- **Al** — A(X) was produced by the RYA-395/398 graded cull on 1 surviving independent-gf line(s) and is HELD UNFROZEN at the ratified gold tier 'owed' (RYA-522: a value is frozen only if we would stake a differential on it). This is a deliberate hold, NOT a zero-survivor cull — the graded firewall did leave lines to stand on. To surface a number the tier must be re-ratified (RYA-522), or the pool broadened via RYA-161/162 (differential survey). Gold note: "LOW_CONFIDENCE / thin graded pool (+0.98)". NLTE grid available (Al_Amarsi2020_PySME.csv).
- **Ca** — A(X) was produced by the RYA-395/398 graded cull on 2 surviving independent-gf line(s) and is HELD UNFROZEN at the ratified gold tier 'owed' (RYA-522: a value is frozen only if we would stake a differential on it). This is a deliberate hold, NOT a zero-survivor cull — the graded firewall did leave lines to stand on. To surface a number the tier must be re-ratified (RYA-522), or the pool broadened via RYA-161/162 (differential survey). Gold note: "LOW_CONFIDENCE / thin graded pool (+0.02)". NLTE grid available (Ca_Mashonkina2017.csv).
- **Na** — A(X) was produced by the RYA-395/398 graded cull on 2 surviving independent-gf line(s) and is HELD UNFROZEN at the ratified gold tier 'owed' (RYA-522: a value is frozen only if we would stake a differential on it). This is a deliberate hold, NOT a zero-survivor cull — the graded firewall did leave lines to stand on. To surface a number the tier must be re-ratified (RYA-522), or the pool broadened via RYA-161/162 (differential survey). Gold note: "LOW_CONFIDENCE / thin graded pool (+0.02)". NLTE grid available (Na_Amarsi2020_PySME.csv).
- **Ni** — A(X) was produced by the RYA-395/398 graded cull on 2 surviving independent-gf line(s) and is HELD UNFROZEN at the ratified gold tier 'owed' (RYA-522: a value is frozen only if we would stake a differential on it). This is a deliberate hold, NOT a zero-survivor cull — the graded firewall did leave lines to stand on. To surface a number the tier must be re-ratified (RYA-522), or the pool broadened via RYA-161/162 (differential survey). Gold note: "LOW_CONFIDENCE / thin graded pool (+0.75)". no NLTE grid (would be LTE-flagged).
...
- **Ti** — A(X) was produced by the RYA-395/398 graded cull on 10 surviving independent-gf line(s) and is HELD UNFROZEN at the ratified gold tier 'owed' (RYA-522: a value is frozen only if we would stake a differential on it). This is a deliberate hold, NOT a zero-survivor cull — the graded firewall did leave lines to stand on. To surface a number the tier must be re-ratified (RYA-522), or the pool broadened via RYA-161/162 (differential survey). Gold note: "LOW_CONFIDENCE / thin graded pool (+0.50)". NLTE grid available (Ti_Mallinson2024_PySME.csv).
...
- **Sr** — A(X) was produced by the RYA-395/398 graded cull on 1 surviving independent-gf line(s) and is HELD UNFROZEN at the ratified gold tier 'owed' (RYA-522: a value is frozen only if we would stake a differential on it). This is a deliberate hold, NOT a zero-survivor cull — the graded firewall did leave lines to stand on. To surface a number the tier must be re-ratified (RYA-522), or the pool broadened via RYA-161/162 (differential survey). Gold note: "+2.13 — NOT a gf-floor; saturated-line-on-flat-COG signature (the RYA-520 disease) → saturation-trace owed". NLTE grid available (Sr_Bergemann2012_INSPECT.csv).
- **Zr** — solar EW measured + matched in linelist_solar, but the RYA-398 graded-gf firewall (wired into the default run, RYA-456) culls every line — zero independent-gf survivors (pool gf is Kurucz/ungraded, or every line fails SAT/HIERR/BLEND). gf-data-limited → RYA-161/162 (differential survey). no NLTE grid (would be LTE-flagged).
- **Ba** — MEASURED via HFS-resolved synthesis — the EW path SAT-culls the strong Ba II 5853 (REW -4.90, at the knee, +HFS/isotope; registry owed). A(Ba)_LTE 2.439 (bsyn COG inverting the observed solar EW 74.62 mA, 23 VALD3 HFS components, total loggf -1.0) + Engine-A Korotin2015 delta -0.0285 (solar 5853 node; validate-don't-tune) = A(Ba) 2.410 (+0.140 vs Asplund 2.27). Off no-value. Engine-B (Gerber atom.ba111 TS-native NLTE) corroborates at the departure level (Ba II 4554 delta -0.018, RYA-534) — 5853 is absent from the GES level-ID block so Engine-A drives the delta; both agree Ba II NLTE is small-negative, so A is engine-insensitive. NOT PASS: the pool EW carries blend_flag=True (~10 mA over the clean solar line ~64 mA), inflating A by ~+0.15 — a blend/gf floor (RYA-161/162), NOT real Ba enhancement. Clean-EW cross-check (64.0 mA->2.187; 66.0 mA->2.231) reconciles with Asplund, confirming the offset is the blend. Deblend owed; do NOT tune.
- **Y** — solar EW measured + matched in linelist_solar, but the RYA-398 graded-gf firewall (wired into the default run, RYA-456) culls every line — zero independent-gf survivors (pool gf is Kurucz/ungraded, or every line fails SAT/HIERR/BLEND). gf-data-limited → RYA-161/162 (differential survey). no NLTE grid (would be LTE-flagged).
...
- **Eu** — solar EW measured + matched in linelist_solar, but the RYA-398 graded-gf firewall (wired into the default run, RYA-456) culls every line — zero independent-gf survivors (pool gf is Kurucz/ungraded, or every line fails SAT/HIERR/BLEND). gf-data-limited → RYA-161/162 (differential survey). no NLTE grid (would be LTE-flagged). Eu II 6645 EW 6.8 mA, ew_integrity disposition=RECOVERED (RYA-102/458 HFS-summing).
